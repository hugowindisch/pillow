...
define.meat = {
    ...
    createApplicationDomain: function (parentDomain) {
        return { packages: {}, modules: {}, parentDomain: parentDomain || this.getTopmostDomain() };
    },
    ...
    getTopmostDomain: function () {
        ...
        if (!td) {
            td = this.topmostDomain = { packages: {}, modules: {}, parentDomain: null };
            tdp = td.packages;
    ...
    loadDependencies: function (pack, cb, optionalApplicationDomain) {
        ...
        if (l > 3) {
            for (i = 3; i < l; i += 1) {
                this.loadPackage(deps[i], onLoaded, optionalApplicationDomain);
            }
        } else {
            cb(null);
        }
    ...
    // load a package
    loadPackage: function (packageName, cb, optionalApplicationDomain) {
        optionalApplicationDomain = optionalApplicationDomain || this.getTopmostDomain();
        var script,
            body,
            that = this,
            pack = this.findPackage(optionalApplicationDomain, packageName);
        ...
        if (!pack) {
            ...
            script.onload = function () {
                // if we have an application domain, we should move the loaded
                // packages to this application domain.
                if (optionalApplicationDomain) {
                    optionalApplicationDomain.packages[packageName] = pack;
                }
                // we should load the dependencies
                that.loadDependencies(pack, onComplete, optionalApplicationDomain);
            };
